...
Pole-to-pole utility spans use their paid cable length and full temperature-dependent conductor resistance, plus endpoint contact resistance. Old saved spans are recalculated on loading. They have a lumped thermal/failure model for one active core, not separate multicore circuits or spatial hot spots. See the [wire thermal model](wire-thermal-model.md) for heat accounting, cooling assumptions, damage and saved-state behavior.
