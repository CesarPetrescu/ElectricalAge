...
class GridCableCompatibilityTest {
    ...
    @Test fun thresholdIsInclusiveAndInvalidRatingsNeverQualify() {
        for (rating in listOf(-1.0, 0.0, 600.0, 800.0, 999.999, Double.NaN,
            Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY)) {
            assertFalse(GridCablePolicy.accepts(rating, false, false), "$rating")
        }
        for (rating in listOf(1000.0, 1000.001, 5000.0, 150000.0)) {
            assertTrue(GridCablePolicy.accepts(rating, false, false))
            assertFalse(GridCablePolicy.accepts(rating, true, false))
            assertFalse(GridCablePolicy.accepts(rating, false, true))
        }
    }

    @Test fun everyRegisteredGridDeviceUsesTheSameRatingRuleForEveryCable() {
        var accepted = 0
        var rejected = 0
        for (device in devices()) for (cable in cables()) {
            val rating = if (cable is UtilityCableDescriptor && cable.insulated)
                cable.insulationVoltageRating else cable.electricalNominalVoltage
            val expected = !cable.signalWire && !(cable is UtilityCableDescriptor && cable.melted) &&
                rating.isFinite() && rating >= 1000.0
    ...
    @Test fun newHvSpoolsWorkWithoutOldPoleFlagsAnd600VoltFlagsDoNotBypassThreshold() {
        ...
        for (cable in newHv) {
            assertFalse(cable.poleEligible, "Fixture must exercise the missing historic flag")
            devices().forEach { assertTrue(it.acceptsGridCable(cable)) }
        }
        val old600 = UtilityCableDescriptor.allDescriptors().filter {
            !it.melted && it.insulated && it.poleEligible && it.insulationVoltageRating == 600.0
        }
        assertTrue(old600.isNotEmpty())
        old600.forEach { cable -> devices().forEach { assertFalse(it.acceptsGridCable(cable)) } }
        val native800 = cables().filter { it !is UtilityCableDescriptor && !it.signalWire && it.electricalNominalVoltage == 800.0 }
        assertTrue(native800.isNotEmpty())
        native800.forEach { cable -> devices().forEach { assertFalse(it.acceptsGridCable(cable)) } }
    }
        ...
        for ((i, aDesc) in descriptors.withIndex()) for ((j, bDesc) in descriptors.withIndex()) {
            ...
            for (facing in listOf(Direction.XN, Direction.XP, Direction.ZN, Direction.ZP)) {
                ...
                for (aSide in aSides) for (bSide in bSides) {
                    GridLink.validateNewLink(a, b, aSide, bSide, hv(), 6)
                    GridLink.validateNewLink(b, a, bSide, aSide, hv(150000.0), 6)
                }
    ...
    @Test fun directApiCannotBypassCableSpanAndDamageValidation() {
        ...
        val low = UtilityCableDescriptor.allDescriptors().first { it.insulated && !it.melted && it.insulationVoltageRating < 1000.0 }
        for (cable in listOf(low, checkNotNull(hv().meltedDescriptor))) {
    ...
    @Test fun existingSaved600VoltLinksRetainTheirCableAndPaidLength() {
        ...
        assertFalse(GridCablePolicy.accepts(cable))
