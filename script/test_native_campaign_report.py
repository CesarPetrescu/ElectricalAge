...
class NativeReportGateTest(unittest.TestCase):
    def setUp(self):
        ...
        for name in self.names+self.assigned:
            f=f'{name}.png';im.save(self.root/f)
            rows.append(dict(id=name,status='passed',kind='gallery' if name in self.assigned else 'functional',observation={'unitData':True},screenshot=f,components=['eln:unit_fixture_2']))
        ...
        self.coverage=dict(runId=self.nonce,jarSha256=self.sha,registry=[dict(id=e['id'],descriptor=e['descriptor']) for e in self.gallery],galleryAll=self.gallery,assignedGallery=self.assigned)
        self.write()
    def write(self):
    ...
    def test_accepts_exact_nonempty_evidence(self):
        r=self.valid();self.assertEqual(r['functional'],6);self.assertEqual(r['gallery'],1);self.assertEqual(len(r['captures']),7)
