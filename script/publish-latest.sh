...
Passed build, unit tests, benchmarks, wire fault/thermal contracts, cooler contracts, companion integrations, server/restart/client smoke tests and both packaged multiplayer profiles: [CI run]($GITHUB_SERVER_URL/$GITHUB_REPOSITORY/actions/runs/$GITHUB_RUN_ID).
